...
def make_startTime(tStart,nTotal,nRest,iti,itiRest):
    ...
    # Set the increment for random elements
    random_increment = itiRest

    # Set the indices for random elements
    random_indices = np.random.choice(length, nRest, replace=False)

    # Generate the array
    for i in range(1, length):
        if i in random_indices:
            trial_time_array[i] = trial_time_array[i - 1] + random_increment
        else:
